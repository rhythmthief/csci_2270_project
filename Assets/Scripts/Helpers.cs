...
internal class Helpers
{
	...
	internal string concatenateData(int _id, Graph gameGraph, bool mode, List<string> namesList)
	{
		string data = "";
		Vertex active = gameGraph.getVertex(_id);
		...
		if (active.fieldAccess[0] || mode == true)
		{
			data += "Name:\t\t" + active.name + "\n";

			if (mode == true && namesList != null)
				namesList.Add(active.name); //Adding names to a list passed by reference
		}
		else
			data += "Name:\t" + "[UNKNOWN]\n";

		if (active.fieldAccess[1] || mode == true)
			data += "Country:\t" + active.country + "\n";
		else
		...
		for (int i = 2; i < 5; i++)
		{
			if (active.fieldAccess[i] || mode == true)
				data += active.misc[i - 2] + "\n";
			else
				data += "[UNKNOWN]\n"; ;
		}

		//Lastly, I'll get all clues generated by this vertex, assuming that it has been correctly guessed and we're generating data for the menu
		if (active.fieldAccess[0] && mode == false)
		{
			data += "Knows someone who:\n";
			data += active.clueShown.getAllKeys() + "\n";
		}
